...
function SBTCard({ sbt, index }: { sbt: SBT; index: number }) {
    ...
    >
      ...
      >
        ...
        >
          ...
          <p className="text-indigo-400 text-[10px]">획득 {sbt.earnedAt}</p>
        </div>
      </motion.div>
    </motion.div>
  );
    ...
    <div className="min-h-screen bg-[var(--background)]">
      ...
      <div className="sticky top-0 z-40 bg-[var(--background)]/90 backdrop-blur-xl border-b border-[var(--card-border)]">
        <div className="max-w-lg mx-auto px-4 py-3">
          <h1 className="text-white font-bold text-lg">💎 내 보상</h1>
          <p className="text-[var(--muted)] text-xs">투표 기여 보상 및 SBT 컬렉션</p>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-4 pb-28 space-y-4">
        ...
        <div className="bg-gradient-to-br from-indigo-900/60 to-purple-900/60 border border-indigo-700/40 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-indigo-300 text-xs mb-1">보유 VTX</p>
              <motion.p
                key={displayBalance}
            ...
            <div className="text-right">
              <p className="text-indigo-300 text-xs mb-1">{MOCK_USER.rankIcon} 등급</p>
              <p className="text-white font-bold text-lg">{MOCK_USER.rank}</p>
              <p className="text-indigo-400 text-xs">투표 {MOCK_USER.totalVotes}회</p>
            </div>
          </div>
          ...
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-3 flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-yellow-300 text-xs font-semibold">미지급 보상</p>
            ...
            >
              ...
              {claimed ? "수령 완료!" : claiming ? "처리 중..." : "수령하기"}
            </motion.button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: "총 획득", val: "1,284 VTX", icon: TrendingUp, color: "text-emerald-400" },
            { label: "이번 주", val: "390 VTX", icon: Star, color: "text-indigo-400" },
            { label: "지갑 주소", val: MOCK_USER.address, icon: Lock, color: "text-[var(--muted)]" },
        ...
        <div className="bg-[var(--card)] border border-[var(--card-border)] rounded-2xl p-4">
          <h2 className="text-white font-semibold text-sm mb-3">주간 보상 추이</h2>
        ...
        <div className="bg-[var(--card)] border border-[var(--card-border)] rounded-2xl p-4">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-white font-semibold text-sm">SBT 컬렉션</h2>
              <p className="text-[var(--muted)] text-xs">카드를 탭하면 상세 정보를 볼 수 있어요</p>
            </div>
          ...
          <div className="grid grid-cols-3 gap-2.5">
            {MOCK_SBTS.map((sbt, i) => (
              <SBTCard key={sbt.id} sbt={sbt} index={i} />
            ))}
          </div>
